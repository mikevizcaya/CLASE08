...
if "resultados_fase2" not in st.session_state:
    st.session_state.resultados_fase2 = None
...
st.caption(
    "Proceso completo: selección de datos → Markowitz → interpretación."
)
...
if st.session_state.fase == 1:
    p1.success("① Fase 1 — Selección")
    p2.info("② Fase 2 — Markowitz")
    p3.info("③ Fase 3 — Interpretación")
elif st.session_state.fase == 2:
    p1.info("① Fase 1 — Completada")
    p2.success("② Fase 2 — Markowitz")
    p3.info("③ Fase 3 — Interpretación")
else:
    p1.info("① Fase 1 — Completada")
    p2.info("② Fase 2 — Completada")
    p3.success("③ Fase 3 — Interpretación")
...
# ============================================================
    ...
    # Guardar resultados necesarios para la Fase 3.
    st.session_state.resultados_fase2 = {
        "comparison": comparison.copy(),
        "comparison_show": comparison_show.copy(),
        "sharpe_weights": sharpe_weights.copy(),
    ...
    if st.button(
        "Siguiente → Interpretar resultados",
        type="primary",
        use_container_width=True,
    ):
...
# ============================================================
    ...
    resultados = st.session_state.resultados_fase2

    if resultados is None:
        st.warning(
            "Primero es necesario completar la Fase 2."
        )
    ...
    comparison = resultados["comparison"].copy()
    comparison_show = resultados["comparison_show"].copy()
    sharpe_weights = resultados["sharpe_weights"].copy()
    min_weights = resultados["min_weights"].copy()
    sectors = resultados["sectors"].copy()
    risk_free_pct = resultados["risk_free_pct"]
    ...
    # --------------------------------------------------------
    st.subheader(
        "3.1 ¿Cuál es el portafolio más eficiente?"
    )

    sharpe_row = comparison.loc[
        comparison["Portafolio"] == "Máximo Sharpe"
    ].iloc[0]

    min_row = comparison.loc[
        comparison["Portafolio"] == "Mínima varianza"
    ].iloc[0]

    equal_row = comparison.loc[
        comparison["Portafolio"] == "Equal Weight"
    ].iloc[0]

    sharpe_ret = sharpe_row["Rendimiento esperado"] * 100
    sharpe_vol = sharpe_row["Volatilidad"] * 100
    sharpe_ratio = sharpe_row["Sharpe"]

    min_ret = min_row["Rendimiento esperado"] * 100
    min_vol = min_row["Volatilidad"] * 100
    min_sharpe = min_row["Sharpe"]

    equal_ret = equal_row["Rendimiento esperado"] * 100
    equal_vol = equal_row["Volatilidad"] * 100
    equal_sharpe = equal_row["Sharpe"]

    st.success(
        f"**Bajo el criterio de rendimiento ajustado por riesgo, "
        f"el portafolio de Máximo Sharpe es el más eficiente de los "
        f"tres portafolios destacados.** Presenta un rendimiento anual "
        f"esperado de **{sharpe_ret:.2f}%**, una volatilidad de "
        f"**{sharpe_vol:.2f}%** y una razón Sharpe de "
        f"**{sharpe_ratio:.3f}**."
    )

    st.write(
        "Esto no significa que exista un único portafolio 'mejor' para "
        "todas las personas. En Markowitz, la elección depende del nivel "
        "de riesgo que el inversionista esté dispuesto a aceptar. "
        "Sin embargo, si el criterio elegido es obtener la mayor cantidad "
        "de rendimiento excedente por unidad de volatilidad, el punto de "
        "**Máximo Sharpe** es la referencia principal."
    )

    # --------------------------------------------------------
    # 3.2 QUÉ SIGNIFICA CADA TÉRMINO
    # --------------------------------------------------------
    st.divider()
    st.subheader(
        "3.2 Interpretación detallada de los conceptos"
    )
...
La **razón Sharpe** mide cuánto rendimiento adicional obtiene el
portafolio por cada unidad de riesgo asumido:
...
En este análisis se utilizó una tasa libre de riesgo de
**{risk_free_pct:.2f}% anual**.

El portafolio de Máximo Sharpe busca hacer esta razón lo más grande
posible. Por eso no intenta simplemente conseguir el mayor rendimiento:
busca un **equilibrio entre rendimiento y riesgo**.

En nuestros resultados:

- Rendimiento esperado: **{sharpe_ret:.2f}%**
- Volatilidad: **{sharpe_vol:.2f}%**
- Sharpe: **{sharpe_ratio:.3f}**

Un Sharpe mayor indica una compensación histórica más favorable por el
riesgo asumido.
"""
    )
...
El portafolio de **Mínima Varianza** tiene otro objetivo: encontrar la
combinación de activos con la **menor volatilidad posible**, respetando
las restricciones del modelo.

No trata de maximizar el rendimiento. Trata de reducir el movimiento
esperado del portafolio.

En nuestros resultados:

- Rendimiento esperado: **{min_ret:.2f}%**
- Volatilidad: **{min_vol:.2f}%**
- Sharpe: **{min_sharpe:.3f}**

Por eso este portafolio puede ser atractivo para alguien cuyo objetivo
principal sea reducir riesgo, aunque renuncie a parte del rendimiento
esperado.
"""
    )
...
La **Frontera Eficiente** no es un solo portafolio. Es el conjunto de
portafolios que Markowitz considera eficientes.

Un portafolio está sobre la frontera cuando:

- para su nivel de riesgo, **no existe otro portafolio con mayor
  rendimiento esperado**, o
- para su nivel de rendimiento, **no existe otro portafolio con menor
  riesgo**.

Los puntos que quedan por debajo o hacia la derecha de esa frontera son
ineficientes, porque existe otra combinación que ofrece más rendimiento,
menos riesgo, o ambas cosas.

El portafolio de Mínima Varianza marca aproximadamente el extremo de
menor riesgo de la frontera. El portafolio de Máximo Sharpe representa
un punto particularmente importante de esa frontera porque maximiza el
rendimiento excedente por unidad de volatilidad.
"""
    )
...
**Equal Weight** significa repartir el capital por igual entre todas las
acciones seleccionadas.

Con 20 acciones:

\[
Peso\ por\ acción = \frac{{1}}{{20}} = 5\%
\]

Este método **no optimiza** usando volatilidades ni covarianzas. Sirve
como referencia para comprobar si Markowitz realmente aporta algo frente
a una estrategia extremadamente sencilla.

En nuestros resultados, Equal Weight produjo:

- Rendimiento esperado: **{equal_ret:.2f}%**
- Volatilidad: **{equal_vol:.2f}%**
- Sharpe: **{equal_sharpe:.3f}**

Compararlo con Máximo Sharpe permite observar si la optimización consiguió
una mejor relación histórica entre rendimiento y riesgo.
"""
    )
    ...
    delta_ret = sharpe_ret - equal_ret
    delta_vol = sharpe_vol - equal_vol
    delta_sharpe = sharpe_ratio - equal_sharpe

    c1, c2, c3 = st.columns(3)

    c1.metric(
        "Diferencia de rendimiento vs Equal Weight",
        f"{delta_ret:+.2f} pp",
    )

    c2.metric(
        "Diferencia de volatilidad vs Equal Weight",
        f"{delta_vol:+.2f} pp",
    )
    ...
    if delta_ret > 0 and delta_vol < 0:
        st.success(
            "En esta muestra histórica, el portafolio de Máximo Sharpe "
            "domina al Equal Weight: ofrece simultáneamente mayor "
            "rendimiento esperado y menor volatilidad."
        )
    elif delta_sharpe > 0:
        st.info(
            "El portafolio de Máximo Sharpe presenta una mejor relación "
            "rendimiento-riesgo que Equal Weight, aunque la mejora implica "
            "un intercambio entre rendimiento y volatilidad."
        )
    else:
        st.info(
            "En esta muestra, Equal Weight resultó competitivo frente al "
            "portafolio optimizado. Esto también sería un resultado válido "
            "para discutir en la tarea."
        )

    # --------------------------------------------------------
    # 3.4 PESOS DEL PORTAFOLIO MÁS EFICIENTE
    # --------------------------------------------------------
    st.divider()
    st.subheader(
        "3.4 Composición del portafolio de Máximo Sharpe"
    )

    st.write(
        "Estos son los pesos que el modelo asignó a los activos que "
        "realmente participan en el portafolio de Máximo Sharpe. "
        "Las acciones con peso prácticamente cero no se muestran."
    )
    ...
    tech_weight = sharpe_weights.loc[
        sharpe_weights["Sector"] == "Tecnología",
        "Peso",
    ].sum()

    ind_weight = sharpe_weights.loc[
        sharpe_weights["Sector"] == "Industriales",
        "Peso",
    ].sum()
    ...
    # --------------------------------------------------------
    # 3.5 DIVERSIFICACIÓN SECTORIAL
    # --------------------------------------------------------
    st.divider()
    st.subheader(
        "3.5 ¿Ayudó combinar Tecnología e Industriales?"
    )

    sectors_show = sectors.copy()
    ...
    best_sector_row = sectors.loc[
        sectors["Sharpe máximo"].idxmax()
    ]

    combined_row = sectors.loc[
        sectors["Universo"] == "Tecnología + Industriales"
    ].iloc[0]

    st.write(
        "Esta tabla permite comprobar si agregar un segundo sector "
        "realmente mejoró la diversificación. La evidencia relevante no "
        "es simplemente que haya más acciones, sino si el universo "
        "combinado logra reducir volatilidad, mejorar el Sharpe o ampliar "
        "la frontera eficiente."
    )

    if best_sector_row["Universo"] == "Tecnología + Industriales":
        st.success(
            "En esta muestra histórica, el universo combinado obtuvo el "
            "Sharpe máximo más alto de las alternativas comparadas, lo que "
            "sugiere que la diversificación entre ambos sectores aportó "
            "valor al balance rendimiento-riesgo."
        )
    else:
        st.info(
            f"En esta muestra histórica, el Sharpe máximo más alto fue el "
            f"de **{best_sector_row['Universo']}**. Esto significa que "
            f"agregar el segundo sector no necesariamente mejoró ese criterio "
            f"específico, aunque sí puede modificar la volatilidad mínima y "
            f"la forma de la frontera eficiente."
        )

    # --------------------------------------------------------
    # 3.6 CONCLUSIÓN PARA LA TAREA
    # --------------------------------------------------------
    st.divider()
    st.subheader(
    ...
    st.markdown(
        f"""
El modelo de Markowitz permitió pasar de una selección de acciones basada
solamente en su desempeño histórico a una construcción de portafolio que
también considera **riesgo y relaciones entre los activos**.

Dentro de los tres portafolios destacados, el de **Máximo Sharpe** fue el
más eficiente bajo el criterio de rendimiento ajustado por riesgo, con
un rendimiento esperado de **{sharpe_ret:.2f}%**, volatilidad de
**{sharpe_vol:.2f}%** y Sharpe de **{sharpe_ratio:.3f}**.

El portafolio de **Mínima Varianza** mostró la alternativa de menor riesgo,
mientras que **Equal Weight** funcionó como una referencia sencilla para
evaluar si la optimización aportaba valor.

La principal aportación de Markowitz no consiste en encontrar simplemente
las acciones que más subieron, sino en determinar cómo combinarlas de
acuerdo con sus rendimientos, volatilidades y covarianzas.
"""
    )

    st.warning(
        "Esta interpretación describe un ejercicio histórico. Las acciones "
        "fueron seleccionadas después de observar su desempeño de cinco años, "
        "por lo que existe sesgo retrospectivo y de supervivencia. El "
        "portafolio de Máximo Sharpe identificado aquí no debe interpretarse "
        "como una predicción de cuál será el portafolio óptimo en el futuro."
    )
